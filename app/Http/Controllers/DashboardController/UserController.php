        ...
        try{
            ...
            if($bid_item_validate === true){
                ...
                if($bidable_amount>= $bid_price&& $get_user_balence> $get_user_bidding_expences){
                    $bid_item_price = AuctionItem::join('seller_product_lists', 'auction_items.serial', 'seller_product_lists.serial')
                    ->where([
                        ['auction_items.serial', '=', $serial],
                        ['auction_items.token', '=', $token],
                        ['auction_items.bidding_staus', '=', 'live'],
                    ])
                    ->whereNull('auction_items.status')->value('starting_price');
                    if($bid_price>$bid_item_price){
